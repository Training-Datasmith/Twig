...
class Enum_Function extends Function_Expression
{
    public function compile(Compiler $compiler): void
    {
        ...
        if (!\is_string($value)) {
            throw new Syntax_Error('The first argument of the "enum" function must be a string.', $this->get_template_line(), $this->get_source_context());
        }
        if (!enum_exists($value)) {
            throw new Syntax_Error(\sprintf('The first argument of the "enum" function must be the name of an enum, "%s" given.', $value), $this->get_template_line(), $this->get_source_context());
        }
        if (!$cases = $value::cases()) {
            throw new Syntax_Error(\sprintf('The first argument of the "enum" function must be a non-empty enum, "%s" given.', $value), $this->get_template_line(), $this->get_source_context());
        }
